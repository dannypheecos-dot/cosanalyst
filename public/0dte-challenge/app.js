/* CosAnalyst 0DTE Challenge — blotter renderer. */
(function () {
  "use strict";
  ...
  function renderOdteDashboard(data) {
    var dash = data && data.dashboard;
    if (!dash) return;
    odteSetText("odte-d-start", odteMoneyInt(dash.startingCapital));
    odteSetText("odte-d-equity", odteMoneyInt(dash.currentEquity));
    odteSetText("odte-d-pnl", odteMoneyInt(dash.realizedPnl));
    odteSetText(
      "odte-d-ret",
      dash.returnPct == null ? "—" : (dash.returnPct >= 0 ? "+" : "") + dash.returnPct + "%"
    );
    odteSetText("odte-d-completed", String(dash.completedTrades != null ? dash.completedTrades : "—"));
    odteSetText(
      "odte-d-wl",
      (dash.rulesBasedWins != null ? dash.rulesBasedWins : 0) +
        " / " +
        (dash.rulesBasedLosses != null ? dash.rulesBasedLosses : 0) +
        " rules-based"
    );
    odteSetText("odte-d-winrate", dash.winRateLabel || "NOT MEANINGFUL — small sample");
    odteSetText(
      "odte-d-avgwin",
      dash.avgWinner != null ? odteMoneyInt(dash.avgWinner) : dash.avgWinnerLabel || "—"
    );
    odteSetText(
      "odte-d-avglos",
      dash.avgLoser != null ? odteMoneyInt(dash.avgLoser) : "—"
    );
    odteSetText("odte-d-pf", dash.profitFactorLabel || "NOT YET MEANINGFUL");
    odteSetText("odte-d-dd", odteMoneyInt(dash.maxDrawdown));
    odteSetText(
      "odte-d-ddpct",
      dash.maxDrawdownPct == null ? "—" : dash.maxDrawdownPct + "% of starting capital"
    );
    odteSetText("odte-d-open", dash.openTradeLabel || "No open position");
    var peak = dash.peakOpportunityStats || {};
    odteSetText(
      "odte-d-peak",
      "Charted +25%: " +
        (peak.chartedPlus25 != null ? peak.chartedPlus25 : "—") +
        " · Verified executable peaks: " +
        (peak.verifiedExecutablePeaks != null ? peak.verifiedExecutablePeaks : "0")
    );
    odteSetText("odte-d-sample", dash.sampleSizeNote || "");
  }
